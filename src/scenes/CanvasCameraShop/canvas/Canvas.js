...
import { Stage, Layer, Group, Text, Rect, Transformer, RegularPolygon, Image, Arrow, Line } from "react-konva";
...
const WS_API = 'https://ai.hasaki.vn/control/check_light_is_on/config'
...
export default class Canvas extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.props.ai_control != prevProps.ai_control || this.props.fullLayer != prevProps.fullLayer ) {
            this.setState({
                fullDatasLayer : this.props.fullLayer,
        ...
        if (Object.keys(ai_control).length) {
            ...
            if (ai_control?.TYPE == 'POLYGON') {
                if (ai_control.PARAMS.type == 'polygon') {
                    this.setState({ isOnArrow: false, isPolygon: true, isFinished: true,
            ...
            if(ai_control_status == 0 ){
                this.setState({ disabledForm: true   , timeStartStop : ai_control.OPERATING_TIME_LIST})
            }
        }else{
    ...
    async submitForm() {
        ...
        if(checkNotSave.length){
            ...
            const json = JSON.stringify(fullDatasLayer)
            const formData = new FormData();
            const formDataStatus = new FormData();
            formData.append('key', 'ai_control');
            ...
            formDataStatus.append('key', 'ai_control_status');
            ...
            showNotify('Notification', 'Updated thành công')
        }
        ...
        // let response = await submitCanvas(formData)
        // let responseStatus = await submitCanvas(formDataStatus)
        // showNotify('Notification', 'Updated thành công')
        // selfThis.props.cbUpdateInfoCam(values)
    }
    ...
    async submitTurnOff() {
        ...
        // values ={
        //     ...values,
        //     OPERATING_TIME_LIST : this.state.timeStartStop
        // }
        // delete values.time
        ...
        // }
        // let paramsStatus = {
        //     key: 'ai_control_status',
        //     value: 0,
        //     IP: this.props.IP,
        //     port: this.props.port,
        //     C: this.props.channel
        // }
        // let response = await submitCanvas(params)
        // let responseStatus = await submitCanvas(paramsStatus)
        // showNotify('Notification', 'Turn Off thành công')
        let {fullDatasLayer} = this.state
    ...
    formCountGuest() {
        ...
        return (
            <>
                ...
                >
                    <PageHeader title={`AI CONTROL ${this.props.indexLayer  ? 'Layer ' + this.props.indexLayer  : '' }`}
                    />
                    <Row>
                        <Col span={12}>
                            <Form.Item name='IGNORE_TIME' label='Ignore time (seconds)'>
                                <InputNumber disabled={this.state.disabledForm == true ? true : false} controls={false} style={{ width: '20%' }}
                            ...
                            <Form.Item name='type' label='Type camera'>
                                <Dropdown datas={typeCamera} defaultOption="-- All type --"  
                                    disabled={this.state.disabledForm == true ? true : false} style={{ width: '40%' }} onChange={() => this.checkChangeForm()}
                                    // mode='multiple'
                                />
                            </Form.Item>
                        </Col>
                    ...
                    <div className="d-flex">
                        <Button type="primary" htmlType='submit' className={this.state.disabledForm == true ? 'd-none' : ''} style={{ width: '10%' }}>Submit</Button>
                        {
                            ...
                            this.state.disabledForm == false ?
                                ...
                                : <Button className="ml-auto mr-3" type='primary'
                                    onClick={() => {this.setState({disabledForm : false })}}>
                                    Turn On
                                </Button>
                        }
                    ...
                    <div>
                            {
                                this.state.isOnRetangle == false ?
                                    <Button
                                        className='ml-2 mt-2'
                                        type='primary'
                                        onClick={() => this.setState({ isOnRetangle : true, isPolygon: false , disabledForm : false})}

                                    >show retangle</Button>
                                    :
                                    <Button className='ml-2' type='primary' onClick={() => this.setState({ isOnRetangle: false})} >
                                        Off retangle
                                    </Button>
                        ...
                        <div className="mt-2">
                            {
                                this.state.isPolygon == false ?
                                    ...
                                    :
                                    <>
                                        <Button className='ml-2' type='primary' onClick={() => this.setState({ isPolygon: false , modeType : '' })} >
                                            Off Polygon
                                        </Button>
