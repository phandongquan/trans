...
import { getByStaff as getRelationByStaff, create, update, destroy , verify} from '~/apis/company/staff/specialized';
...
class StaffSpecialized extends Component {
    /**
     * 
     * @param {*} props 
     */
    ...
    async getData(params = {}) {
         ...
         * @fetch API
         */
        let specializedXhr = await getRelationByStaff(id , params);
        let specializeds = (specializedXhr.status == 1) ? specializedXhr.data.rows : {};
        let types = specializedXhr.data.types
        this.setState({ specializeds, types}, () => this.setState({ loading: false }));
    }
    ...
    render() {
        ...
        const columns = [
            ...
            {
              title : t('hr:name') + '/' + t('hr:code') + '/' + t('place'),  
              render : r => {
                return <div>
                    <strong>{r.name}</strong> - <span>{t('hr:code') }: {r.code}</span><br/>
                ...
                render: r => {
                    ...
                    if(r.images && (r?.images.length)){
                        (r.images).map((img , index) => {
                            result.push(<Image src={MEDIA_URL_HR + img} className='mr-2' key={index} style={{width : '50px' , height:'50px'}}/>)
                        })
                    }
                    return result
                }
            ...
            //     render: r =>
            //         r.verify_date
            //             ? <span>{r.verify_date} <strong>by #{r.verify_by}</strong></span>
            //             : ""
        ...
        return (
            <>
                <PageHeader title={t('Add specialized for staff')} 
                tags={
                ...
                <Row className="card p-3 pt-0 mb-3">
                    ...
                    {/* <Row className='pt-2'>
                        <Col span={4}>
                            <Dropdown datas={this.state.types} 
                            value={this.state.valueType} 
                            defaultOption='--All types--' 
                            onChange = {v => this.searchType(v)} 
                            onClear={v => this.setState({valueType : null})}/>
                        </Col>
                    </Row> */}
                </Row>
                <Row gutter={[16, 24]}>
                ...
                >
                    <Form
                        ...
                        <Row gutter={24}>
                            ...
                            <Col xs={24} sm={24} md={24} lg={8} xl={8}>
                                <FormItem label={t('Place')} name="place" hasFeedback rules={[{ required: true, message: t('Please input phone place') }]}>
                                    <Dropdown datas={cities} defaultOption="-- All Place --" />
                                </FormItem>
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={6} xl={6}>
                                <FormItem label={t('Type')} name="type" hasFeedback rules={[{ required: true, message: t('Please input type') }]}>
                                    <Dropdown datas={this.state.types} defaultOption="-- All Types --" />
                                </FormItem>
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={5} xl={5}>
                                <FormItem label={t('Supply Date')} name="date"  hasFeedback rules={[{ required: true, message: t('Please input date') }]}>
                                    <DatePicker placeholder={t('Supply Date')} style={{ width: '100%' }} />
                                </FormItem>
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={5} xl={5}>
                                <FormItem label={t('Expire Date')} name="expire_date">
                                    <DatePicker placeholder={t('Expire Date')} style={{ width: '100%' }} />
                                </FormItem>
                            </Col>
                            <Col span={24}>
                                <Form.Item
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslation()(StaffSpecialized));
