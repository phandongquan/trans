...
import { getESLog, getCustomerSearch, deleteES, postSearchByFace, postImageRegister } from '~/apis/aiFaceLog'
import debounce from 'lodash/debounce';
import LazyLoad from 'react-lazy-load';
import ListCustomerCount_v2 from './ListCustomerCount_v2';
import HistoryCashierUse_v2 from './HistoryCashierUse_v2';
...
class ListCustomer extends Component {
    ...
    submitImage() {
        ...
        postSearchByFace(formData).then(function (response) {
            //handle success
            if (response.log_list?.length) {
                selfThis.setState({ loadingModalImg: false, datas: response.log_list, total: (selfThis.state.limit * response.maxpage), visibleModalImg: false })
            } else {
    ...
    submitFormAdd(values) {
        ...
        formData.append('image', this.state.file[0])
        postImageRegister(formData).then(function (response) {
            //handle success
            if (response) {
                ...
                render: (r, text, index) =>
                    staff_info.staff_dept_id == 133 ?
                        <>
                            ...
                            <Button className='mr-2' type="primary" size='small' icon={<FontAwesomeIcon icon={faPen} />} onClick={() => this.openModalEdit(r)} />
                            <DeleteButton onConfirm={(e) => this.onDeleteImg(e, r.id, index)} />
                        </>
                        : []
            }
        ]
                    ...
                    <>
                        <Col span={3}>
                            <Form.Item name="is_customer" >
                                <Dropdown datas={typeCustomer} defaultOption={t('hr:all_type')} />
                            </Form.Item>
                        </Col>
                        <Col span={4}>
                            <Form.Item name="view" >
                                <Dropdown datas={{ not_count: 'Face success', count_only: 'Face fail', all: 'All' }} defaultOption={t('views')} />
                            </Form.Item>
                        </Col>
                        <Col span={3}>
                            <Form.Item name="site" >
                                <Dropdown datas={{ spa: 'Spa' }} defaultOption={t('hr:mode_site')} />
                            </Form.Item>
                        </Col>
                        <Col span={3}>
                            <Form.Item name="area_type" >
                                <Dropdown datas={{ 0: t('normal'), 1: t('hr:cashier') }} defaultOption={t('hr:area_type')} />
                            </Form.Item>
                        </Col>
                    </>
                    <Col span={4}>
                        ...
                        <Button type="primary" style={{ marginLeft: 4 }} onClick={() => this.setState({ visibleModalImg: true })}>
    ...
    renderoptionCount() {
        return <ListCustomerCount_v2
            ...
            onSearchCustomer_id={(customer_id, valuesForm = {}) => {
                ...
                window.scrollTo(0, 0)
                this.setState({ page: 1, loading: true, option: optionDefault }, () => {
    ...
    render() {
        ...
        return (
            <div>
                <PageHeader title={t('hr:face_log_debug') + 'v2'}
                    tags={[
                        <Button
                            key="create-user"
                            type="primary"
                ...
                <Row gutter={[16, 24]}>
                    <Col span={24} className='card p-3'>
                        <Tabs className='pb-3 pl-3' activeKey={String(this.state.option)} onChange={value => this.setState({ option: value })}>
                            <Tabs.TabPane tab={t('face_log')} key={optionDefault}>
                            </Tabs.TabPane>
                            <Tabs.TabPane tab={t('hr:count')} key={optionCount} >
                            </Tabs.TabPane>
                            <Tabs.TabPane tab={t('hr:cashier_use')} key={optionCashierUse}>
                            </Tabs.TabPane>
                        </Tabs>
                        {this.state.option == optionDefault ? this.renderOptionDefault()
                            : this.state.option == optionCount ? this.renderoptionCount()
                ...
                >
                    <Spin spinning={this.state.loadingModalImg}>
                        ...
                        >
                            {
                                imageUrl ?
                                    <img src={imageUrl} alt="Thumbnail" style={{ width: "160px", height: '140px' }} />
                                    :
                                    <>
                                        <p className="ant-upload-drag-icon">
                                            <InboxOutlined />
                                        </p>
                                        <p className="ant-upload-text">{t('hr:drap_drop_upload_image')}</p>
                ...
                </Modal>
                {
                    this.state.visibleModalEdit ?
                        <ModalEditCustomer
                            ...
                            onCancel={() => this.setState({ visibleModalEdit: false, dataDetail: {} })}
                            onSubmitSuccess={() => {
                                let values = this.formRef.current.getFieldsValue();
                                this.getListFaceFilter(values)
                        ...
                        <Form
                            ...
                            <Row>
                                ...
                                <Col className='mt-2' span={24}>
                                    <Form.Item name='user_id' hasFeedback rules={[
                                        {
                                            required: true,
                            ...
                            <Row>
                                <Col span={24}>
                                    ...
                                    <Col className='mt-2' span={24} >
                                        <Form.Item name={'user_type'} hasFeedback rules={[
                                            {
                                                required: true,
                            ...
                            <Row>
                                <Col span={24}>
                                    ...
                                    <Col className='mt-2' span={24} >
                                        <Form.Item name={'shop_id'} hasFeedback rules={[
                                            {
                                                required: true,
